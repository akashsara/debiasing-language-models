...
class DistilBertLM(transformers.BertPreTrainedModel):
    def __init__(self, pretrained_model):
        pass
...
class AlbertLM(transformers.BertPreTrainedModel):
    def __init__(self, pretrained_model):
        pass
...
class ModelNSP(nn.Module):
    def __init__(self, pretrained_model, model_class,  nsp_dim=300):
        ...
        self.pretrained2model = {"xlnet": "XLNetModel", "bert": "BertModel",
                                 "roberta": "RobertaModel", "gpt2": "GPT2Model", "t5": "T5Model", "distilbert": "DistilBertModel", 'albert': 'AlbertModel'}
        self.model_class = self.pretrained2model[model_class.lower().split(
            "-")[0]]
        print("Model Classes: ", self.model_class)
        self.core_model = getattr(
            transformers, self.model_class).from_pretrained(pretrained_model)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, next_sentence_label=None,
        ...
        if 'gpt2' in self.model_class.lower():
            ...
            logits = self.tokenizer.batch_decode(
